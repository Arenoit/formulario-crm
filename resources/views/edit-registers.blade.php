...
<?php
    $id = request()->id;
    $formuser = \App\Models\FormUser::find($id); // o usar where si necesitas más control
?>

    <form class="js-validate" action="{{ route('edit-user',$formuser->id) }}" method="post" enctype="multipart/form-data"
        id="form-id">
        @csrf
        @method('PUT')
                            ...
                            <div class="mb-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="nombre" id="nombre" class="form-control __form-control"
                                        placeholder="nombre completo" value="{{ $formuser->nombre }}" required>
                                </div>
                            </div>
                            <div class="mb-4">
                            ...
                            <div class="mb-4">
                                <div class="form-group mb-0">
                                    ...
                                    <textarea type="text" id="direccion" name="direccion"
                                        ...
                                        class="form-control __form-control h-120" required>{{ $formuser->direccion }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <input type="number" id="telefono" name="telefono" class="form-control __form-control"
                                    placeholder="teléfono" value="{{ $formuser->telefono }}" required>
                            </div>
                        </div>
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <select name="sexo" id="sexo" required
                                    ...
                                    <option value="F" {{ $formuser->sexo == 'F' ? 'selected' : '' }}>Femenino</option>
                                    <option value="M" {{ $formuser->sexo == 'M' ? 'selected' : '' }}>Masculino</option>
                                </select>
                            </div>
                        </div>
                        <div class="mb-4">
                            <div class="form-group">
                                ...
                                <input type="date" id="fecha_nacimiento" name="fecha_nacimiento" class="form-control __form-control"
                                    placeholder="fecha de nacimiento" value="{{ $formuser->fecha_nacimiento }}" required>
                            </div>
                        </div>
<?php
...
?>
                        <div class="form-group mb-4">
                            ...
                            <select name="provincia" id="provincia" required
                                ...
                                @foreach ($provinces as $province)
                                    <option value="{{ $province->id }}" {{ $formuser->provincia_id == $province->id ? 'selected' : '' }}>{{ $province->nombre }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="text-end pt-3">
            <a href="{{ route('registers') }}" class="cmn--btn rounded-md border-0 text-white px-3 py-2" style="background-color: #6c757d;">Regresar</a>
